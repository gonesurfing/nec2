...
def convert_fixed_to_free(lines):
    """Convert fixed-form Fortran to free-form."""
    result = []
    ...
    for line in lines:
        ...
        # Check for continuation (column 6)
        if len(line) > 5 and line[5] not in ' 0':
            # This is a continuation line - append to previous with &
            if result:
                prev = result[-1]
                if not prev.rstrip().endswith('&'):
                    result[-1] = prev.rstrip() + ' &'
            # Remove the continuation marker and leading spaces
            content = line[6:].lstrip() if len(line) > 6 else ''
        ...
        # Regular statement - extract from columns 7-72
        if len(line) > 6:
            # Get label (columns 1-5)
            label = line[:5].strip()
            # Get statement (columns 7+)
            stmt = line[6:72] if len(line) > 6 else ''

            if label:
                result.append(label + ' ' + stmt.rstrip())
            else:
                result.append('  ' + stmt.rstrip())
        else:
            result.append(line)
...
def convert_to_lowercase(lines):
    ...
    for line in lines:
        ...
        for i, s in enumerate(strings):
            code_part = code_part.replace(f'__string_{i}__', s)
...
def extract_and_convert():
    ...
    # Line numbers are 1-based from the file
    routines = [
